...
def get_sub_dirs(rootdir, out_list):
...
def ng_batch_detect():
    ...
    if not os.path.exits(dstdir):
        os.mkdir(dstdir)

    idx1 = 0
    out_list = []
    get_sub_dirs(rootdir, out_list)
    if debug_print:
        print("--dirs--")
        for lst in out_list:
            pt_path, req_id, f_dir = lst
            print(f_dir)
        print("--/dirs--")
    ...
    for lst in out_list:
        ...
        if debug_print:
            ...
            print("----")
        idx1 += 1
        fn_res = os.path.join(dstdir
                , req_id + "_" + str(res_js["ng"]) + '_result_%5d.json' % idx1)  

        with open(fn_res, 'w') as outfile:
            json.dump(res_js, outfile)

    return {"dstdir": dstdir, "total": len(out_list),  "out_list": out_list}



if __name__ == '__main__':
    print('API SERVER RUN')
    run(app, port=8081, debug=True, reloader=False)
